...
class CurationEngine:
    ...
    def curate_server_day(self, server_name: str, date_str: str, messages: List[Dict]) -> List[Dict]:
        ...
        # Optimize for 30k TPM Limit
        # Instead of slow batching, we process the latest ~200 messages (~20k tokens safe zone).
        # This captures all recent news while dropping older backlog (acceptable for digest).
        messages.sort(key=lambda x: x['timestamp'], reverse=True)
        messages = messages[:200]
        
        print(f"  Processing top {len(messages)} most recent messages...")
